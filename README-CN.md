...
- **节点管理**：支持批量对节点重命名、删除或配置监听端口；可查看每个节点最近一次测活延迟。
- **节点筛选**：可在分流组高级模式中按导入源、地区和自定义规则筛选节点。
- **节点测活**：基于 mihomo 内核，支持按源自动测活与源级/节点级/整体手动测活，只把延迟合格的节点输出到下游订阅。
...
- 测活使用生产镜像内置的固定版本 mihomo 内核（v1.19.28，amd64/arm64）；源码/非官方镜像部署时，请确保 `mihomo` 在 PATH 中，或通过 `MIHOMO_PATH` 指定二进制。
- 开启自动测活后，保存订阅以及每次手动/定时刷新都会测活；失败节点保留在“节点管理”，不会写入下游订阅。
- 若提示“未找到 mihomo 内核”，请检查 `MIHOMO_PATH` 或镜像内的 mihomo 二进制。
